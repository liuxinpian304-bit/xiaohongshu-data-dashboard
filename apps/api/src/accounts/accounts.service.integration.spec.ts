...
describe('account credential lifecycle', () => {
  ...
  it('fails closed when direct calls try unavailable authorization semantics',async()=>{const service=new AccountsService(new AuditService());for(const connectorType of ['mock','official','self_import','schema-test'])await expect(service.authorize({connectorType,platformId:crypto.randomUUID(),secret:'x',kind:'oauth'})).rejects.toMatchObject({status:403});for(const connectorType of ['official','self_import']){const account=await prisma.account.create({data:{connectorType,platformId:crypto.randomUUID()}});await expect(service.deactivate(account.id)).rejects.toMatchObject({status:403});await expect(service.reauthorize(account.id,'x','oauth')).rejects.toMatchObject({status:403});await expect(service.remove(account.id,true)).rejects.toMatchObject({status:403});}});
