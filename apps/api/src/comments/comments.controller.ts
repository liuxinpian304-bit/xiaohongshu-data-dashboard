...
export class CommentsController {
  ...
  private filter(query: CommentQueryDto): CommentFilter {
    ...
    return { platform: query.platform, accountId: query.accountId ? uuid(query.accountId, 'accountId') : undefined, accountIds: query.accountIds, noteId: query.noteId ? uuid(query.noteId, 'noteId') : undefined, from, to, keyword: query.keyword, newOnly: query.newOnly === 'true' };
  }
