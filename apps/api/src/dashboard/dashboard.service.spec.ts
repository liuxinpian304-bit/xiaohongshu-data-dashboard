...
describe('DashboardService', () => {
  ...
  it('does not invent a cross-version cumulative increase across a transition gap', async () => {
    ...
    expect(result.trend.at(-1)?.metrics.find(({ key }) => key === 'likes')?.availability).toBe('not_synced');
  });
  it('uses the same complete definition segments for the final card and trend value', async () => {
    const store = storeWith([]);
    store.read = async () => ({ definitions: [
      { id: 'v1', key: 'likes', displayName: '点赞', aggregation: 'cumulative_delta', effectiveFrom: new Date('2025-12-01'), effectiveTo: new Date('2025-12-30T00:00:00Z') },
      { id: 'v2', key: 'likes', displayName: '点赞', aggregation: 'cumulative_delta', effectiveFrom: new Date('2025-12-30T00:00:00Z'), effectiveTo: null },
    ], snapshots: [
      snap({ metricDefinitionId: 'v1', value: '10' }),
      snap({ metricDefinitionId: 'v1', value: '15', capturedAt: new Date('2025-12-29T23:59:59.999Z') }),
      snap({ metricDefinitionId: 'v2', value: '100', capturedAt: new Date('2025-12-30T00:00:00Z') }),
      snap({ metricDefinitionId: 'v2', value: '107', capturedAt: new Date('2026-01-02T12:00:00Z') }),
    ], lastSyncedAt: new Date() } as any);
    const result = await new DashboardService(store).get('weekly', undefined, 'official', new Date('2026-01-05T04:00:00Z'));
    const card = result.cards.find(({ key }) => key === 'likes');
    const finalTrend = result.trend.at(-1)?.metrics.find(({ key }) => key === 'likes');
    expect(card).toMatchObject({ value: '12', availability: 'available' });
    expect(finalTrend).toEqual(card);
  });
  ...
  it('applies explicit interval, period-end and safe deduplicated semantics', async () => {
    ...
    const windowStart = new Date('2025-12-28T16:00:00.000Z'); const windowEnd = new Date('2026-01-04T16:00:00.000Z');
    const result = await new DashboardService(storeWith([
