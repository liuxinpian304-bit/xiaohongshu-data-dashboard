...
export class PrismaDashboardStore implements DashboardStore {
  ...
  async read(periodStart: Date, periodEnd: Date, source: string, accountId: string | undefined, now: Date) {
    ...
    const [definitions, inPeriod, baselines, lastSync] = await Promise.all([
      prisma.metricDefinition.findMany({ orderBy: { key: 'asc' }, select: { id: true, key: true, displayName: true, aggregation: true } }),
    ...
    const map = (snapshot: (typeof inPeriod)[number]): DashboardSnapshot => ({
      ...
      metricDefinitionId: snapshot.metricDefinitionId, metricKey: snapshot.metricDefinition.key, aggregation: snapshot.metricDefinition.aggregation, availability: snapshot.availability,
...
function seriesDeltas(snapshots: DashboardSnapshot[], start: Date, cutoff: Date): DeltaSnapshot[] {
  ...
  return [...groups.values()].map((items) => {
    ...
    return { ...end, delta: aggregateMetricSeries(aggregation, sequence.map((item) => ({ value: Number(item.value) }))) };
  });
}
...
export class DashboardService {
  ...
  async get(period: string, accountId?: string, source = SOURCE, now = new Date()) {
    ...
    for (const item of data.snapshots) if (!definitionMap.has(item.metricKey)) definitionMap.set(item.metricKey, { id: item.metricDefinitionId, key: item.metricKey, displayName: item.metricKey, aggregation: item.aggregation });
