...
@Injectable() export class JobsService {
  ...
  async create(accountId:string){if(!(await prisma.account.count({where:{id:accountId}})))throw new NotFoundException('managed account not found');const id=crypto.randomUUID();const job=await prisma.syncJob.create({data:{id,externalJobId:id,accountId}});await this.audit.record('sync.started','SyncJob',job.id,{accountId});return job}
