...
export default async function NotesPage({ searchParams }: { searchParams: Promise<{ platform?: 'xiaohongshu'|'douyin'; accountId?: string; cursor?: string }> }) {
  ...
  return <div className="workflow-page"><header className="workflow-heading"><div><h1>作品数据</h1><p>集中查看小红书笔记与抖音作品的核心指标。</p></div><div className="note-page-actions"><NoteExport accountId={p.accountId} platform={p.platform}/><form className="inline-filter"><label>平台<select name="platform" defaultValue={p.platform ?? ''}><option value="">全部平台</option><option value="xiaohongshu">小红书</option><option value="douyin">抖音</option></select></label><label htmlFor="note-account">账号</label><select id="note-account" name="accountId" defaultValue={p.accountId ?? ''}><option value="">全部账号</option>{accounts.status === 'ok' ? accounts.data.items.map((a) => <option key={a.id} value={a.id}>{accountLabel(a)}</option>) : null}</select><button>查看</button></form></div></header>
