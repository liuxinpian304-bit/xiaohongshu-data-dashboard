...
import { BffRequestError, parseAdminSession, readBoundedJson, validateMutationRequest, webCsrfCookie } from '../../../../lib/bff';
...
export async function POST(request: Request) {
  ...
  if(login.ok){try{const values=(login.headers as Headers&{getSetCookie?:()=>string[]}).getSetCookie?.()??(login.headers.get('set-cookie')?[login.headers.get('set-cookie')!]:[]);response.cookies.set({name:'admin_session',value:parseAdminSession(values),httpOnly:true,secure:process.env.NODE_ENV==='production',sameSite:'strict',path:'/',maxAge:8*60*60});}catch{return NextResponse.json({error:'invalid upstream session'},{status:502})}}
