...
const metricLabels = { views: '播放/阅读', likes: '点赞', comments: '评论', favorites: '收藏' };
...
export function NoteExplorer({ notes }: { notes: Note[] }) {
  ...
  return <section className="note-explorer">
    <div className="note-explorer-toolbar" aria-label="作品展示方式">
      <span>共 {notes.length} 条作品</span>
    ...
    {view === 'table' ? <div className="note-table-wrap"><table className="note-table">
      <thead><tr><th>作品</th><th>平台</th><th>账号</th><th>发布时间</th>{metricKeys.map((key) => <th key={key}>{metricLabels[key]}</th>)}<th>数据来源</th></tr></thead>
      <tbody>{notes.map((note) => <tr key={note.id}>
        <td><Link href={`/notes/${note.id}`}>{note.title}</Link><small>ID {note.platformId}</small></td>
        <td><span className="note-source">{note.platform === 'douyin' ? '抖音' : '小红书'}</span></td><td>{note.account.displayName || note.account.platformId}</td><td>{formatShanghaiDateTime(note.publishedAt)}</td>
      ...
      <div className="note-data-card-head"><div><Link href={`/notes/${note.id}`}>{note.title}</Link><small>{note.platform === 'douyin' ? '抖音' : '小红书'} · {note.account.displayName || note.account.platformId} · {formatShanghaiDateTime(note.publishedAt)}</small></div><span className="note-source">{sourceLabel(note.connectorType)}</span></div>
