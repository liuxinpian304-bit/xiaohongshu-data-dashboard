...
type SessionStatus = { state: SessionState; changedAt: string; qrExpiresAt?: string; errorCode?: string };
...
const labels: Record<SessionState, string> = {
  ...
  verification_required: '需要你完成平台验证', expired: '二维码已过期', closed: '登录窗口已关闭', error: '登录服务异常',
};
...
export function SelfImportLogin() {
  ...
  return <section className="xhs-login-card" aria-labelledby="self-login-title">
    ...
    <div className="action-row">
      {!status || ['idle', 'closed', 'expired', 'error'].includes(status.state) ? <button type="button" disabled={pending} onClick={() => void action('start')}>在驾驶舱登录小红书</button> : null}
