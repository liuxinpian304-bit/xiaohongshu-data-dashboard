...
export function SettingsOverview({ status }: { status: SettingsStatus }) {
  ...
  return <div className="settings-overview">
    ...
    <section className="settings-section"><div className="settings-section-heading"><div><h2>数据源</h2><p>不同来源的数据会始终分开标记。</p></div></div><div className="settings-source-grid"><article><strong>账号自抓数据</strong><span>当前真实数据入口，读取你本人已登录账号的笔记和评论。</span></article><article><strong>官方 API</strong><span>接口入口已保留，平台审批通过后启用。</span></article></div></section>
