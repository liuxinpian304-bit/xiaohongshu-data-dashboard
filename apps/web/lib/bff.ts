...
import { primaryAllowedOrigin, requireAllowedOrigin } from '@xhs/domain';
...
export function cookieSecureForOrigin(origin: string) { return new URL(origin).protocol === 'https:'; }
...
export async function forwardMutation(request: Request, path: string, method: 'POST'|'PATCH'|'DELETE' = 'POST', allowedFields:string[] = [], maxBytes=16_384) {
  let origin:string;try { origin=validateMutationRequest(request); } catch { return NextResponse.json({ error: '请求来源不可信' }, { status: 403 }); }
  ...
  const upstream = await fetch(`${apiBase}${path}`, { method, headers: mutationHeaders(session, csrf,origin), body: JSON.stringify(value) });
