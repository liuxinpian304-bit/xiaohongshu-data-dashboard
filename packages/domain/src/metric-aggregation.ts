...
export function aggregateMetricSeries(aggregation: MetricAggregation, series: readonly AggregationValue[], period?: AggregationPeriod): number | null {
  ...
  if (aggregation === 'sum_interval') {
    ...
    if (ordered[0]!.windowStart!.getTime() !== period.start.getTime() || ordered.at(-1)!.windowEnd!.getTime() !== period.endExclusive.getTime()) return null;
  ...
  if (aggregation === 'period_end') return series.filter((item) => item.authoritativePeriod && period && item.windowEnd?.getTime() === period.endExclusive.getTime()).at(-1)?.value ?? null;
